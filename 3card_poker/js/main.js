...
var CANVAS_WIDTH = 1700,
  ...
  TEXT_EVALUATOR =
    "STRAIGHT FLUSH;THREE OF A KIND;STRAIGHT;FLUSH;ONE PAIR;HIGH CARD;NO HAND".split(
      ";"
    ),
...
function CGfxButton(a, d, b, c) {
  ...
  this.buttonDown = function () {
    g || (n[ON_MOUSE_DOWN] && n[ON_MOUSE_DOWN].call(p[ON_MOUSE_DOWN], h));
  };
  this.setPosition = function (a, b) {
    q.x = a;
    q.y = b;
...
function CGame(a) {
  ...
  this.setBet = function (a, b) {
    if (y.isResultPanelvisible())
      y.disableBetFiches(), e.clearBet(), (D = this.setBet), this._onEndHand();
    else {
  ...
  this.setPairPlusBet = function (a) {
    if (y.isResultPanelvisible())
      y.disableBetFiches(),
        e.clearBet(),
        (D = this.setPairPlusBet),
        this._onEndHand();
    else {
      a = FICHES_VALUE[a];
      var b = e.getBetPlus() + a;
      b = parseFloat(b.toFixed(2));
      0 === e.getBetAnte() && e.getCredit() - a <= 3 * FICHES_VALUE[0]
  ...
  this._calculateTotalWin = function () {
    ...
    0 < e.getBetPlus() &&
      k <= ONE_PAIR &&
      (t = e.getBetPlus() * PAYOUT_PLUS[k] + e.getBetPlus());
    h = 0;
  ...
  this.onRebet = function () {
    y.isResultPanelvisible() && ((D = this.rebet), this._onEndHand());
  };
  this.onDeal = function () {
    ...
    if (e.getBetAnte() < MIN_BET)
      ...
      U.removeAllChildren();
      if (
        (F < a ? WIN_OCCURRENCE + 1 : Math.floor(101 * Math.random())) >
        WIN_OCCURRENCE
      ) {
        do {
        ...
        } while (q === NO_HAND || "player" === w || "dealer_no_hand" === w);
      } else {
        do
          (R = this._generateRandPlayerCards()),
            (N = this._generateRandDealerCards()),
    ...
    if (!1 !== d)
      switch (f) {
        case STATE_GAME_WAITING_FOR_BET:
          c += s_iTimeElaps;
          6e3 < c &&
            ((c = 0), O.isVisible() || 0 !== e.getBetAnte() || O.show(1));
          break;
        case STATE_GAME_DEALING:
          this._updateDealing();
          break;
...
function CInterface(a) {
  ...
  this._init = function (a) {
    ...
    t = new CGfxButton(830, CANVAS_HEIGHT - x.height / 2, x, s_oStage);
    t.addEventListener(ON_MOUSE_UP, this._onButClearRelease, this);
    x = s_oSpriteLibrary.getSprite("but_rebet");
    l = new CGfxButton(890, CANVAS_HEIGHT - x.height / 2, x, s_oStage);
    l.disable();
    ...
    T = new CPaytablePanel(CANVAS_WIDTH - 303, 400, s_oStage);
    this.disableButtons();
    this.refreshButtonPos(s_iOffsetX, s_iOffsetY);
  };
  this.unload = function () {
...
function CFichesController() {
  ...
  this.refreshFiches = function (a, c, d) {
    ...
    for (var g = c, f = d + 10, l = (b = 0), m = 0; m < a.length; m++)
  ...
  this.createFichesPile = function (a, b, c) {
    ...
    do {
      ...
      for (var h = 0; h < f; h++)
        d.push({
          value: g,
          index: s_oGameSettings.getIndexForFiches(g),
        });
      ...
      switch (d) {
        ...
        case THREE_OF_A_KIND:
          return a[2].rank > c[2].rank
            ...
            : "standoff";
        case ONE_PAIR:
          for (f = d = 0; f < a.length - 1; f++)
            if (a[f].rank === a[f + 1].rank) {
              d = a[f].rank;
              break;
            }
          for (f = a = 0; f < c.length - 1; f++)
        ...
        case HIGH_CARD:
          return (
            (c = c[2].rank),
            (a = a[2].rank),
            a > c ? "player" : a < c ? "dealer" : "standoff"
          );
        default:
          return "standoff";
      }
