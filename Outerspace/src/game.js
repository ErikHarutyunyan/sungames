...
let JACKPOT = localStorage.getItem(GAME_NAME + '_user_jackpot');
let CREDITS = localStorage.getItem(GAME_NAME + '_user_credits');
let FS = localStorage.getItem(GAME_NAME + '_opt_fs');
let SFX = localStorage.getItem(GAME_NAME + '_opt_sfx');
let MUSIC = localStorage.getItem(GAME_NAME + '_opt_music');
let LANGUAGE = localStorage.getItem(GAME_NAME + '_opt_language');
if (JACKPOT === null) {
...
BasicGame.GameLoad.prototype = {
    ...
    preload: function() {
        this.game.load.image('bg1', GAME_PATH + 'assets/bg1.webp');
        this.game.load.image('bg2', GAME_PATH + 'assets/bg2.webp');
        this.game.load.image('bg3', GAME_PATH + 'assets/bg3.webp');
        this.game.load.image('bg4', GAME_PATH + 'assets/bg4.webp');
        this.game.load.image('bg5', GAME_PATH + 'assets/bg5.webp');
        this.game.load.image('preloader_logo', GAME_PATH + 'assets/loading.webp');
    },
    create: function() {
        ...
        let logo = this.game.add.sprite(200, 400, 'preloader_logo');
        logo.x = this.game.world.centerX - (logo.width / 2);
        let bar = this.LoaderBar('408', '40', this.game.world.centerX - 204, 600, '#f3e619', '#2872e0');
    ...
    BeginLoad: function() {
        this.load.atlasJSONHash('basis', GAME_PATH + 'assets/basis_high.webp', GAME_PATH + 'assets/basis_high.json');
        this.load.atlasJSONHash('symbols', GAME_PATH + 'assets/symbols.webp', GAME_PATH + 'assets/symbols.json');
        this.load.atlasJSONHash('anim', GAME_PATH + 'assets/animation.webp', GAME_PATH + 'assets/animation.json');
    ...
    LoaderBar: function(width, height, pos_x, pos_y, Color, BarColor) {
        let progress = game.add.group();

        let barbg = game.add.bitmapData(width, height);
        barbg.ctx.beginPath();
        barbg.ctx.rect(0, 0, width, height);
        barbg.ctx.fillStyle = Color;
        barbg.ctx.fill();

        let bar = game.add.bitmapData(width - 6, height - 6);
        bar.ctx.beginPath();
...
BasicGame.Game.prototype = {
    ...
    create: function() {
        ...
        this.logo = this.game.add.sprite(this.game.world.centerX, 0, 'basis', 'logo.webp');
        ...
        let sym_begin = this.sym_pos_x; // X Cord. begin of the first Slot Symbol
        let h1_begin = 280; // X End from Button (left)
        let h2_begin = h1_begin;
        let h1_end = 1450; // X Begin from button (right)
        let h2_end = h1_end;
        let sym_v = 0; // Y Begin of the first Line
        let sym_v_steps = 63; // Y space from buttons
        let h_begin_now = h2_begin;
        let h_end_now = h2_end;
        let sym_pos = 0;
        ...
        // Animation Left
        this.robot = this.add.sprite(20, 600, 'anim', 'Fire2_000.webp');
        this.robot.scale.set(1.5);
        this.robot.animations.add('Fire', Phaser.Animation.generateFrameNames('Fire2_', 0, 19, '.webp', 3), 20, false, false);
        this.robot.animations.add('Attack', Phaser.Animation.generateFrameNames('Attack_', 0, 19, '.webp', 3), 20, false, false);
        this.RoboRand = [];
        for (let i = 0; i < 20; i++) {
            this.RoboRand.push(0);
        }
        for (let i = 0; i < 10; i++) {
            this.RoboRand.push('Fire');
        }
        for (let i = 0; i < 10; i++) {
            this.RoboRand.push('Attack');
        }
        this.shuffle(this.RoboRand);
        this.time.events.loop(Phaser.Timer.SECOND * 30, this.RoboAction, this);

        this.menu = this.add.sprite(0, 0, 'basis', 'menu.webp');
        this.gamebuttons = this.add.group();
        this.autobutton = this.gamebuttons.create(710, 990, 'basis', 'btn_auto_en.webp');
        this.startbutton = this.gamebuttons.create(835, 980, 'basis', 'btn_play.webp');
        this.settingsbutton = this.gamebuttons.create(925, 990, 'basis', 'btn_options_en.webp');
        this.infobutton = this.gamebuttons.create(1065, 990, 'basis', 'btn_info_en.webp');
        this.betless = this.gamebuttons.create(450, 1014, 'basis', 'btn_less.webp');
        this.betmore = this.gamebuttons.create(602, 1014, 'basis', 'btn_more.webp');
        ...
        // labels
        this.label_jp = this.add.sprite(20, 973, 'basis', 'label_jackpot_en.webp');
        this.label_credits = this.add.sprite(230, 973, 'basis', 'label_credits_en.webp');
        this.label_bet = this.add.sprite(440, 973, 'basis', 'label_bet_en.webp');
        this.label_msg = this.add.sprite(1285, 973, 'basis', 'label_message_en.webp');
        this.label_fs = this.add.sprite(1765, 973, 'basis', 'label_fs_en.webp');

        // Fields
        this.ValueJackpot = this.game.add.text(115, 1012, this.number_format(JACKPOT, DECIMAL, ',', ''), {
            font: "bold 18px Arial",
            fill: "#ffffff"
        });
        this.ValueCredits = this.game.add.text(320, 1012, this.number_format(CREDITS, DECIMAL, ',', ''), {
            font: "bold 18px Arial",
            fill: "#ffffff"
        });
        this.ValueBet = this.game.add.text(535, 1012, this.number_format(BETS[BET_POS], DECIMAL, ',', ''), {
            font: "bold 18px Arial",
            fill: "#ffffff"
        });
        this.ValueFs = this.game.add.text(1775, 1012, this.number_format(FS[BET_POS], DECIMAL, ',', ''), {
            font: "bold 18px Arial",
            fill: "#ffffff"
        });
        this.ValueMessage = this.game.add.text(1295, 1012, LANG[LANGUAGE]['welcome'], {
            font: "bold 18px Arial",
            fill: "#ffffff"
        });
        ...
        let jpbg = game.add.bitmapData(this.jpbg_width, this.jpbg_height);
        jpbg.ctx.beginPath();
        ...
        let jpbg2 = game.add.bitmapData(this.jpbg_width - 6, this.jpbg_height - 6);
        jpbg2.ctx.beginPath();
        ...
        let helpbg = game.add.bitmapData(this.helpbg_width, this.helpbg_height);
        ...
        let helpbg2 = game.add.bitmapData(this.helpbg_width - 3, this.helpbg_height - 6);
        ...
        this.helpmenu_close_button = this.helpmenubuttons.create(this.helpbg2.x - 10, this.helpbg2.y - 10, 'basis', 'btn_close.webp');
        ...
        let ybegin = this.helpbg.y + 100;
        let xbegin = this.helpbg.x + 40;
        let ypos = ybegin;
        let xpos = xbegin;
        ...
        for (let i = 1; i < 11; i++) {
            ...
            let c = this.helpmenu.create(xpos, ypos, 'symbols', 'sym_' + i + '.png');
            c.scale.setTo(0.5);

            let bg = game.add.bitmapData(175, c.height);
            bg.ctx.beginPath();
        ...
        ybegin = this.helpbg.y + 580;
        xbegin = this.helpbg.x + 337;
        ypos = ybegin;
        xpos = xbegin;
        for (let i = 0; i < WLDATA.length; i++) {
            let wl = this.GenerateHelpWinline(i);
            if (i == 7) {
                ypos += 80;
                xpos = xbegin + 58;
            }
        ...
        let settingsbg = game.add.bitmapData(this.settingsbg_width, this.settingsbg_height);
        ...
        let settingsbg2 = game.add.bitmapData(this.settingsbg_width - 3, this.settingsbg_height - 6);
        ...
        // Buttons
        this.settings_close_button = this.settingsmenubuttons.create(this.settingsmenu.width - 35, -10, 'basis', 'btn_close.webp');
        this.settings_sfx_button = this.settingsmenubuttons.create(300, 110, 'basis', 'btn_opt_activ.webp');
        if (SFX !== true) {
            this.settings_sfx_button.frameName = 'btn_opt_normal.webp';
        }
        this.settings_music_button = this.settingsmenubuttons.create(300, 150, 'basis', 'btn_opt_activ.webp');
        if (MUSIC !== true) {
            this.settings_music_button.frameName = 'btn_opt_normal.webp';
        }
        this.settings_fullscreen_button = this.settingsmenubuttons.create(300, 190, 'basis', 'btn_opt_normal.webp');
        this.settings_lang1_button = this.settingsmenubuttons.create(this.settingsbg_width / 2 - 80, 320, 'basis', 'lang_en.webp');
        this.settings_lang2_button = this.settingsmenubuttons.create(this.settingsbg_width / 2, 320, 'basis', 'lang_de.webp');
        this.settings_lang3_button = this.settingsmenubuttons.create(this.settingsbg_width / 2 + 80, 320, 'basis', 'lang_es.webp');
    ...
    CreateReel: function(x, y, width, height) {
        let reel = this.add.group();
        let sym_y = y + height * 2;
        for (let i = 0; i < 9; i++) {
            let c = this.add.sprite(x, sym_y, 'symbols', 'sym_' + this.game.rnd.between(1, 10) + '.png');
            sym_y -= height;
            if (i > 5) {
                for (let n = 1; n < 11; n++) {
    ...
    createWinlineButtons: function(x, y, height, winlines) {
        let winlinebuttons = this.add.group();
        let space = 2;
        let buttonSize = (height - (winlines * space)) / winlines;
        if (buttonSize > 53) {
            buttonSize = 53;
            let spacer = height - (buttonSize * winlines);
            space = spacer / (winlines - 1);
        }
        for (let i = 1; i <= winlines; i++) {
            let button_container = this.add.group();
            let button = game.add.bitmapData(buttonSize, buttonSize);
            button.ctx.beginPath();
            button.ctx.rect(0, 0, buttonSize, buttonSize);
            button.ctx.fillStyle = '#' + WLDATA[(i - 1)].color;
            button.ctx.fill();
            let label = this.game.add.text(x + button.width / 2, y + ((button.height - (button.height / 1.5)) / 2), i, {
                font: "bold " + (button.height / 1.5) + "px Arial",
                fill: "#ffffff"
            });
    ...
    CreateFaktorLabels: function(xpos, ypos, i) {
        if (i < 9) {
            let t1 = this.game.add.text(xpos + 170, ypos + 10, '3 = ' + LANG[LANGUAGE]['bet'] + ' x' + SYM_FAKTORS[i - 1]['match_3'], {
                font: "bold 18px Arial",
                fill: "#ffffff"
            });
            let t2 = this.game.add.text(xpos + 170, t1.y + 39, '4 = ' + LANG[LANGUAGE]['bet'] + ' x' + SYM_FAKTORS[i - 1]['match_4'], {
                font: "bold 18px Arial",
                fill: "#ffffff"
            });
            let t3 = this.game.add.text(xpos + 170, t2.y + 39, '5 = ' + LANG[LANGUAGE]['bet'] + ' x' + SYM_FAKTORS[i - 1]['match_5'], {
                font: "bold 18px Arial",
                fill: "#ffffff"
            });
            ...
            let t1 = this.game.add.text(xpos + 170, ypos + 10, LANG[LANGUAGE]['desc_fs1_sym'], {
                font: "bold 18px Arial",
                fill: "#ffffff"
            });
            let t2 = this.game.add.text(xpos + 170, t1.y + 39, LANG[LANGUAGE]['desc_fs2_sym'] + ' =', {
                font: "bold 18px Arial",
                fill: "#ffffff"
            });
            let t3 = this.game.add.text(xpos + 170, t2.y + 39, SYM_FAKTORS[8]['match_3'] + ' ' + LANG[LANGUAGE]['desc_fs3_sym'], {
                font: "bold 18px Arial",
                fill: "#ffffff"
            });
            this.faktorlables.add(t1);
            ...
            let t2 = this.game.add.text(xpos + 170, ypos + 49, '5 = ' + LANG[LANGUAGE]['desc_jp_sym'], {
                font: "bold 18px Arial",
                fill: "#ffffff"
            });
            this.faktorlables.add(t2);
    ...
    PressSFX: function() {
        if (SFX === true) {
            ...
            SFX = true;
            this.settings_sfx_button.frameName = 'btn_opt_activ.webp';
        }
        localStorage.setItem(GAME_NAME + '_opt_sfx', SFX);
    },
    PressMusic: function() {
        if (MUSIC === true) {
            MUSIC = false;
            ...
            MUSIC = true;
            MusicBG.play('', 0, 1, true);
            this.settings_music_button.frameName = 'btn_opt_activ.webp';
        }
        localStorage.setItem(GAME_NAME + '_opt_music', MUSIC);
    },
    PressFullScreen: function() {
        if (this.game.scale.isFullScreen) {
            this.game.scale.stopFullScreen();
            this.settings_fullscreen_button.frameName = 'btn_opt_normal.webp';
        } else {
            this.game.scale.startFullScreen(false);
            this.settings_fullscreen_button.frameName = 'btn_opt_activ.webp';
        }
    },
    ...
    UpdateLanguage: function() {
        this.label_jp.frameName = 'label_jackpot_' + LANGUAGE + '.webp';
        this.label_credits.frameName = 'label_credits_' + LANGUAGE + '.webp';
        this.label_bet.frameName = 'label_bet_' + LANGUAGE + '.webp';
        this.label_msg.frameName = 'label_message_' + LANGUAGE + '.webp';
        this.label_fs.frameName = 'label_fs_' + LANGUAGE + '.webp';
        this.autobutton.frameName = 'btn_auto_' + LANGUAGE + '.webp';
        this.settingsbutton.frameName = 'btn_options_' + LANGUAGE + '.webp';
        this.infobutton.frameName = 'btn_info_' + LANGUAGE + '.webp';
        ...
        for (let c in this.faktorlables.children) {
            this.faktorlables.children[c].kill();
        }
        let ybegin = this.helpbg.y + 103;
        let xbegin = this.helpbg.x + 40;
        let ypos = ybegin;
        let xpos = xbegin;
        for (let i = 1; i < 11; i++) {
        ...
        localStorage.setItem(GAME_NAME + '_opt_language', LANGUAGE);
    },
    PressPlay: function() {
    ...
    InitPlay: function() {
        if (CREDITS >= BET || FS[BET_POS] > 0) {
            if (FS[BET_POS] > 0) {
                ...
                JACKPOT = Number(JACKPOT + (BET / 100 * 1));
                CREDITS = Number(CREDITS - BET);
            }
            this.updateFS();
            this.updateJACKPOT();
            this.updateCREDITS();
            ...
            this.poolsyms = [];
            for (let i = 0; i <= 9; i++) {
                for (let n = 0; n < CHANCES[i]['count']; n++) {
            ...
            for (let i = 1; i <= 9; i++) {
                if (this.game.rnd.between(0, PUSHER[i]['chance']) == 1) {
                    for (let n = 0; n < this.ressyms.length; n++) {
    ...
    CreateCoin: function() {
        let pos = this.game.world.randomX;
        this.coin = this.coins.create(pos, -80, 'basis', 'coin_01.webp');
        ...
        this.coin.animations.add('idle', Phaser.Animation.generateFrameNames('coin_', 1, 6, '.webp', 2), 20, true, false);
    ...
    GenerateHelpWinline: function(line) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            for (let n = 0; n < 3; n++) {
                ...
                let sym = game.add.bitmapData(this.settingsbg_width, this.settingsbg_height);
                sym.ctx.beginPath();
    ...
    GenerateLines: function(begin_pos, end_pos, sym_begin, sym_v, sym_v_steps, sym_pos, color, vpos1, vpos2, vpos3, vpos4, vpos5, vdif) {
        let winl = this.add.graphics(begin_pos, sym_v);
    ...
    PlayWalze1: function() {
        let speed = 500;
        let art = Phaser.Easing.Linear.None;
        ...
        this.walze1_ani = this.add.tween(this.walze1).to({
            y: this.walze1.y + (this.sym_height * 6)
        }, speed, art, true);
        if (SFX === true) {
            this.walze1_ani.onStart.add(function() {
                this.WheelStartSFX.play('', 0, 1, true);
            }, this);
        }
    ...
    PlayWalze2: function() {
        ...
        this.walze2_ani = this.add.tween(this.walze2).to({
            y: this.walze2.y + (this.sym_height * 6)
    ...
    PlayWalze3: function() {
        ...
        this.walze3_ani = this.add.tween(this.walze3).to({
            y: this.walze3.y + (this.sym_height * 6)
        }, speed, art, true, delay);
    ...
    PlayWalze4: function() {
        ...
        this.walze4_ani = this.add.tween(this.walze4).to({
            y: this.walze4.y + (this.sym_height * 6)
        }, speed, art, true, delay);
    ...
    PlayWalze5: function() {
        ...
        this.walze5_ani = this.add.tween(this.walze5).to({
            y: this.walze5.y + (this.sym_height * 6)
        }, speed, art, true, delay);
    ...
    CheckWinlines: function(CheckLine) {
        ...
        if (match > 0) {
            let winsym = this.GetSymbolNumber(this.walze1.children[this.Reverse(WLDATA[CheckLine].vpos1 - 0.5)].frameName);
    ...
    updateCREDITS: function() {
        localStorage.setItem(GAME_NAME + '_user_credits', CREDITS);
        this.ValueCredits.text = this.number_format(CREDITS, DECIMAL, ',', '');
    },
    updateFS: function() {
        localStorage.setItem(GAME_NAME + '_user_fs', FS);
        this.ValueFs.text = this.number_format(FS[BET_POS], DECIMAL, ',', '');
    },
    updateJACKPOT: function() {
        localStorage.setItem(GAME_NAME + '_user_jackpot', JACKPOT);
    ...
    RoboAction: function() {
        let rand = this.game.rnd.between(0, this.RoboRand.length);
        if (rand != 0) {
    ...
    update: function() {
        ...
        if (this.coin_ani === true) {
            let r = Math.round(this.jpwin / 500);
            if (r < 1) {
                r = 1;
            }
